...
class FullImageDatamanager(DataManager, Generic[TDataset]):
    ...
    def sample_train_cameras(self):
        ...
        if self.config.train_cameras_sampling_strategy == "random":
            ...
            raise ImportError("fpsample is not installed, please install it to use fps sampling")
        ...
        def undistort_idx(idx: int) -> Dict[str, torch.Tensor]:
            ...
            if data["image"].shape[1] != camera.width.item() or data["image"].shape[0] != camera.height.item():
                warnings.warn(
                    ...
                    "The image will be cropped after distortion correction."
                )
            ...
            image = image[: camera.height.item(), : camera.width.item()]
            mask = mask[: camera.height.item(), : camera.width.item()] if mask is not None else None
    ...
    def fixed_indices_eval_dataloader(self) -> List[Tuple[Cameras, Dict]]:
        ...
        for i in image_indices:
            ...
            _camera = _cameras[i : i + 1]
            _camera.metadata["cam_idx"] = i
